...
async def scrape_google_form(request: ScrapeRequest):
    try:
        ...
        form_data = await loop.run_in_executor(executor, extract_google_form_data, request.url)
        test_id = save_test(
        ...
        return {"test_id": test_id, "title": form_data['title'], "question_count": len(form_data['questions'])}
