...
def save_test(title, url, reading_passage, questions_data):
    """Save extracted test to database"""
    with db_connection() as conn:
        cursor = conn.execute(
...
def get_all_tests():
    """Get all tests with latest session info"""
    with db_connection() as conn:
        tests = conn.execute('''
            SELECT t.*, 
...
def get_test_by_id(test_id):
    """Get test details including all questions (without exposing answers)"""
    with db_connection() as conn:
...
def save_session_results(test_id, time_spent_seconds, answers, time_limit_seconds):
    """Save test session results and return score data"""
    with db_connection() as conn:
...
def get_mistakes_by_test(test_id=None):
    """Get all mistakes, optionally filtered by test"""
    with db_connection() as conn:
        query = '''
            SELECT 
...
def get_distinct_wrong_question_ids(test_id):
    """Get unique question IDs that were answered incorrectly across all sessions of a test"""
    with db_connection() as conn:
        rows = conn.execute('''
...
def get_questions_by_ids(question_ids):
    """Get questions for retesting mistakes"""
    if not question_ids:
        return []
    with db_connection() as conn:
